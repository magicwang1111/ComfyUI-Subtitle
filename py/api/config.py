...
def _validate_oss_group(merged: dict[str, Any]) -> None:
    ...
    if missing:
        raise ValueError(
            "OSS upload config is incomplete; set " + ", ".join(missing) + " in config.local.json or environment variables."
        )
...
def load_tencent_cloud_config(overrides: dict[str, Any] | None = None) -> TencentCloudConfig:
    ...
    cos_bucket = str(_config_or_env_value(merged, "tencent_cos_bucket", "TENCENT_COS_BUCKET", "")).strip()
    oss_endpoint = str(_config_or_env_value(merged, "oss_endpoint", "OSS_ENDPOINT", "")).strip()
    oss_access_key_id = str(_config_or_env_value(merged, "oss_access_key_id", "OSS_ACCESS_KEY_ID", "")).strip()
    oss_access_key_secret = str(_config_or_env_value(merged, "oss_access_key_secret", "OSS_ACCESS_KEY_SECRET", "")).strip()
    oss_bucket = str(_config_or_env_value(merged, "oss_bucket", "OSS_BUCKET", "")).strip()

    if not cos_bucket and not (oss_endpoint and oss_access_key_id and oss_access_key_secret and oss_bucket):
        raise ValueError(
            "At least one storage path must be configured. Set complete OSS config for OSS input, or set tencent_cos_bucket for COS-based flow."
        )

    return TencentCloudConfig(
        secret_id=secret_id,
        secret_key=secret_key,
        region=str(_config_or_env_value(merged, "tencent_region", "TENCENT_REGION", "ap-guangzhou")).strip(),
        ...
        oss_prefix=_normalize_prefix(_config_or_env_value(merged, "oss_prefix", "OSS_PREFIX", "kling_uploads"), "kling_uploads"),
